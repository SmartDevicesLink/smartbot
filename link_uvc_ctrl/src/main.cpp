...
int main(int argc, char **argv) {
    ...
    mosquitto_message_callback_set(g_mosq, on_message_callback);

    if (mosquitto_connect(g_mosq, MQTT_HOST, MQTT_PORT, 60)) {
        spdlog::error("Could not connect to MQTT Broker.");
        return -1;
    }

    // 订阅手动控制主题
    mosquitto_subscribe(g_mosq, nullptr, MQTT_TOPIC, 0);
    // 订阅用于“暂停/恢复”自动追踪的模式控制主题
    mosquitto_subscribe(g_mosq, nullptr, "camera/tracking/set_mode", 0);

    // 创建并启动处理MQTT消息的线程
    std::thread mqtt_thread(mqtt_loop, g_mosq);
